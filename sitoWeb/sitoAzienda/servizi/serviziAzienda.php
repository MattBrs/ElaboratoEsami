    ...
    <body>
        ...
        <div class="content">
            ...
            if($_SESSION['nomeUtente'] != ""){
                ...
                $query = "select * from servizi_azienda";       //leggo tutti i servizi che offre l'azienda

                $statement = $conn->prepare($query);
                $statement->execute();
                $result = $statement->get_result();

                if($result->num_rows){                          //stampo i servizi
                    while($row = $result->fetch_assoc()){
                        echo "<a href='./servizio" . $row['nome_servizio'] . ".php'><h3>" . $row['nome_servizio'] . "</h3></a>";
